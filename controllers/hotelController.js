...
export const getAllHotels = async (req, res) => {
  try {
    ...
    // ⚙️ Վերցնում ենք settings (տոկոսները որպես % թիվ, ոչ թե fraction)
    const settings = await GlobalSettings.findOne({});
    const b2cPct = Number(settings?.b2cMarkupPercentage) || 0;              // %
    const officePct = Number(settings?.officeMarkupPercentage) || 0;        // %
    const defaultSPPct = Number(settings?.defaultSalesPartnerMarkup) || 0;  // %
    ...
    const hotelsWithPrices = hotels.map((hotel) => {
      const net = Number(hotel.minOffer.price.amount);
      const currency = hotel.minOffer.price.currency;
      let finalPrice = net;

      if (role === "b2c" || role === "guest") {
        // guest-ին էլ վերաբերվի որպես B2C (եթե պետք չլինի՝ guest-ը հանեք)
        finalPrice = net * (1 + b2cPct / 100);
      } else if (role === "b2b_sales_partner") {
        const userSPPct = (req.user?.markupPercentage ?? defaultSPPct);  // %
        const individualMarkup = userSPPct / 100;                         // fraction
        finalPrice = net * (1 + individualMarkup);
      } else if (role === "office_user") {
        finalPrice = net * (1 + officePct / 100);
      }
